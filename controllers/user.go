...
func CreateSymptom(w http.ResponseWriter, r *http.Request) {
	...
	json.NewDecoder(r.Body).Decode(symptom)

	token := r.Context().Value("user").(*models.Token)

	user := &models.User{}

	db.First(&user, token.UserID)

	db.Model(&user).Association("Symptoms").Append(symptom)

	createdSymptom := db.Save(symptom)

	var errMessage = createdSymptom.Error

	if createdSymptom.Error != nil {
		fmt.Println(errMessage)
	}

	json.NewEncoder(w).Encode(createdSymptom)
}
...
func GetSymptoms(w http.ResponseWriter, r *http.Request) {
	...
	db.Preload("Symptoms").First(&user, token.UserID)

	json.NewEncoder(w).Encode(user.Symptoms)
}
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	db.Where("user_id = ? ", id).Find(&user.Symptoms) //conditions
